...
Результат: `build/windows/x64/runner/Debug/LoggerFlutterClient.exe`
...
Результат: `build/windows/x64/runner/Release/LoggerFlutterClient.exe`
...
5. **Результат**: `installer/LoggerFlutterClient_Setup.exe`
...
# Инсталлер готов: installer/LoggerFlutterClient_Setup.exe
```
...
│       └── runner/
│           ├── Debug/
│           │   ├── LoggerFlutterClient.exe      (Debug версия)
│           │   └── (зависимости)
│           └── Release/
│               ├── LoggerFlutterClient.exe      (Release версия)
│               └── (зависимости)

installer/
└── LoggerFlutterClient_Setup.exe         (Инсталлер)
```
...
```ini
[Setup]
AppName=LoggerFlutterClient           # Имя приложения
...
OutputBaseFilename=LoggerFlutterClient_Setup   # Имя файла
```
...
dir "build\windows\x64\runner\Release\LoggerFlutterClient.exe"
...
- **Файл**: `installer/LoggerFlutterClient_Setup.exe`
- **Размер**: ~100-150 MB
  ...
  build:
    ...
    steps:
      ...
      - uses: softprops/action-gh-release@v1
        with:
          files: installer/LoggerFlutterClient_Setup.exe
```
...
signtool sign /f mycert.pfx /p password /t http://timestamp.server.com build\windows\x64\runner\Release\LoggerFlutterClient.exe
```
